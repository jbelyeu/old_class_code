...
public class BatchDownloader extends JDialog
{
	...
	class DownloaderPanel extends JPanel
	{
		...
		{
			GetProjectsParams params = new GetProjectsParams (batchState.getUsername(),
																batchState.getPassword());
			
			GetProjectsResult result = facade.getProjects(params, batchState.getHost(),
																batchState.getPort());
	...
	protected void getSampleImage()
	{
		...
		GetSampleImageParams params = new GetSampleImageParams(batchState.getUsername(),
																batchState.getPassword(), ID);
		params.setHost(batchState.getHost());
		params.setPort(batchState.getPort() + "/ServerData/");
		BufferedImage image = facade.getSampleImage(params, batchState.getHost(), 
															batchState.getPort());
	...
	{
		ClientFacade facade = new ClientFacade();
		GetBatchParams params = new GetBatchParams();
		
		params.setHost(batchState.getHost());
		params.setPassword(batchState.getPassword());
		params.setPort(batchState.getPort());
		params.setUsername(batchState.getUsername());
		int id = this.IdsByProject.get(projects.getSelectedItem());
		params.setProjectID(id);
		
		batchState.setBatchInfo(facade.downloadBatch(params, batchState.getHost(),
															batchState.getPort()));

		String url = batchState.getBatchInfo().getImageURL();
		BufferedImage image = facade.getImage(url, batchState.getHost(), batchState.getPort());
		batchState.setImage(image);
		this.dispose();
		System.out.println(batchState.getBatchInfo().toString());		
	}
}
